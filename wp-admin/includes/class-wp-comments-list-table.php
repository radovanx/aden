    ...
    function get_views() {
        ...
        foreach ($stati as $status => $label) {
            ...
            $status_links[$status] = "<a href='$link'$class>" . sprintf(
                            translate_nooped_plural($label, $num_comments->$status), number_format_i18n($num_comments->$status)
                    ) . '</a>';
        }
    ...
    function extra_tablenav($which) {
        ...
        function column_comment($comment) {
            ...
            printf(__('Submitted on <a href="%1$s">%2$s at %3$s</a>'), $comment_url,
                    /* translators: comment date format. See http://php.net/date */ get_comment_date(__('Y/m/d')), get_comment_date(get_option('time_format'))
            );
