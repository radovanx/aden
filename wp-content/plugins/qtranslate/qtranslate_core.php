...
function qtrans_updateGettextDatabases($force = false, $only_for_language = '') {
    ...
    foreach ($q_config['locale'] as $lang => $locale) {
        if (qtrans_isEnabled($only_for_language) && $lang != $only_for_language)
            continue;
...
function qtrans_convertBlogInfoURL($url, $what) {
    if ($what == 'stylesheet_url')
        return $url;
    if ($what == 'template_url')
        return $url;
    if ($what == 'template_directory')
        return $url;
    if ($what == 'stylesheet_directory')
        return $url;
...
function qtrans_join($texts) {
    global $q_config;
    if (!is_array($texts))
        $texts = qtrans_split($texts, false);
...
function qtrans_enableLanguage($lang) {
    ...
    if ($q_config['auto_update_mo'])
        qtrans_updateGettextDatabases(true, $lang);
    return true;
}
