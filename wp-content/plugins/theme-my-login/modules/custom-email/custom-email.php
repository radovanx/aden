...
if (!class_exists('Theme_My_Login_Custom_Email')) :
    ...
    class Theme_My_Login_Custom_Email extends Theme_My_Login_Abstract {
        ...
        protected function load() {
            ...
            //
           // add_action('tml_frozen_user_notification', array(&$this, 'apply_frozen_user_filters'));
        ...
        public function apply_retrieve_pass_filters() {
            $this->set_mail_headers(
                    $this->get_option(array('retrieve_pass', 'mail_from')), $this->get_option(array('retrieve_pass', 'mail_from_name')), $this->get_option(array('retrieve_pass', 'mail_content_type'))
            );
        ...
        public function apply_password_reset_filters() {
            $this->set_mail_headers(
                    $this->get_option(array('reset_pass', 'admin_mail_from')), $this->get_option(array('reset_pass', 'admin_mail_from_name')), $this->get_option(array('reset_pass', 'admin_mail_content_type'))
            );
        ...
        public function apply_frozen_user_filters() {
            add_filter('frozen_user_notification_title', array(&$this, 'frozen_user_notification_title_filter'), 10, 2);
            add_filter('frozen_user_notification_message', array(&$this, 'frozen_user_notification_message_filter'), 10, 3);
            add_filter('send_frozen_user_notification', array(&$this, 'send_frozen_user_notification_filter'));
        }

        public function frozen_user_notification_title_filter() {
            $_title = $this->get_option(array('frozen_user', 'title'));
            return empty($_title) ? $title : Theme_My_Login_Common::replace_vars($_title, $user_id);
        }

        public function frozen_user_notification_message_filter($data) {
            $_message = $this->get_option(array('frozen_user', 'message'));
            if (!empty($_message)) {
        ...
        public function send_frozen_user_notification_filter($enable) {
            // We'll cheat and set out headers here
            $this->set_mail_headers(
                    $this->get_option(array('frozen_user', 'mail_from')), $this->get_option(array('frozen_user', 'mail_from_name')), $this->get_option(array('frozen_user', 'mail_content_type'))
            );
            return $enable;
        }

        public function frozen_user_notification($user_id) {
            global $current_site;
            ...
            $data['user_firstname'] = $user_info->first_name;
            $data['user_lastname'] = $user_info->last_name;

            $message = $this->frozen_user_notification_message_filter($data);            
            $title = $this->frozen_user_notification_title_filter();
            $to = $user_email;

            $from_email = $this->get_option(array('frozen_user', 'mail_from'));
            $from_name = $this->get_option(array('frozen_user', 'mail_from_name')); 
            $content_type = $this->get_option(array('frozen_user', 'mail_content_type'));
        ...
        public function password_change_notification_message_filter($message, $user_id) {
            ...
            $message = empty($_message) ? $message : Theme_My_Login_Common::replace_vars($_message, $user_id);
            return __($message);
        }
        ...
        public function send_new_user_notification_filter($enable) {
            // We'll cheat and set out headers here
            $this->set_mail_headers(
                    $this->get_option(array('new_user', 'mail_from')), $this->get_option(array('new_user', 'mail_from_name')), $this->get_option(array('new_user', 'mail_content_type'))
            );
            return $enable;
        }
        ...
        public function send_new_user_admin_notification_filter($enable) {
            // We'll cheat and set out headers here
            $this->set_mail_headers(
                    $this->get_option(array('new_user', 'admin_mail_from')), $this->get_option(array('new_user', 'admin_mail_from_name')), $this->get_option(array('new_user', 'admin_mail_content_type'))
            );
        ...
        public function apply_user_moderation_notification_filters() {
            ...
            switch ($moderation_type) {
                case 'email' :
                    $this->set_mail_headers(
                            $this->get_option(array('user_activation', 'mail_from')), $this->get_option(array('user_activation', 'mail_from_name')), $this->get_option(array('user_activation', 'mail_content_type'))
                    );
                ...
                case 'admin' :
                    $this->set_mail_headers(
                            $this->get_option(array('user_approval', 'admin_mail_from')), $this->get_option(array('user_approval', 'admin_mail_from_name')), $this->get_option(array('user_approval', 'admin_mail_content_type'))
                    );
        ...
        public function apply_user_approval_notification_filters() {
            $this->set_mail_headers(
                    $this->get_option(array('user_approval', 'mail_from')), $this->get_option(array('user_approval', 'mail_from_name')), $this->get_option(array('user_approval', 'mail_content_type'))
            );
        ...
        public function apply_user_denial_notification_filters() {
            $this->set_mail_headers(
                    $this->get_option(array('user_denial', 'mail_from')), $this->get_option(array('user_denial', 'mail_from_name')), $this->get_option(array('user_denial', 'mail_content_type'))
            );
