...
if (!class_exists('Theme_My_Login_User_Moderation_Admin')) :
    ...
    class Theme_My_Login_User_Moderation_Admin extends Theme_My_Login_Abstract {
        ...
        public function admin_menu() {
            add_submenu_page(
                    'theme_my_login', __('Theme My Login User Moderation Settings', 'theme-my-login'), __('Moderation', 'theme-my-login'), 'manage_options', $this->options_key, array(&$this, 'settings_page')
            );
                ...
                if (in_array('pending', (array) $user_object->roles)) {
                    switch ($this->get_option('type')) {
                        case 'email' :
                            ...
                            $actions['resend-activation'] = sprintf('<a href="%1$s">%2$s</a>', add_query_arg('wp_http_referer', urlencode(esc_url(stripslashes($_SERVER['REQUEST_URI']))), wp_nonce_url("users.php?action=resendactivation&amp;user=$user_object->ID", 'resend-activation')
                                    ), __('Resend Activation', 'theme-my-login')
                            );
                            break;
                        case 'admin' :
                            // Add "Approve" link
                            $actions['approve-user'] = sprintf('<a href="%1$s">%2$s</a>', add_query_arg('wp_http_referer', urlencode(esc_url(stripslashes($_SERVER['REQUEST_URI']))), wp_nonce_url("users.php?action=approve&amp;user=$user_object->ID", 'approve-user')
