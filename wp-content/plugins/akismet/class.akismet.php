...
class Akismet {
    ...
    public static function set_last_comment($comment) {
        if (is_null($comment)) {
            ...
            self::$last_comment = wp_filter_comment(
                    array_merge(
                            array('comment_author_IP' => self::get_ip_address()), $comment
                    )
            );
        }
    }
    ...
    private static function comments_match($comment1, $comment2) {
        ...
        return (
                isset($comment1['comment_post_ID'], $comment2['comment_post_ID']) && intval($comment1['comment_post_ID']) == intval($comment2['comment_post_ID']) && $comment1['comment_author'] == $comment2['comment_author'] && $comment1['comment_author_email'] == $comment2['comment_author_email']
                );
    }
