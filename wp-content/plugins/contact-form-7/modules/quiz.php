...
function wpcf7_quiz_shortcode_handler($tag) {
    ...
    $html = sprintf(
            '<span class="wpcf7-form-control-wrap %1$s"><span class="wpcf7-quiz-label">%2$s</span>&nbsp;<input %3$s /><input type="hidden" name="_wpcf7_quiz_answer_%4$s" value="%5$s" />%6$s</span>', sanitize_html_class($tag->name), esc_html($question), $atts, $tag->name, wp_hash($answer, 'wpcf7_quiz'), $validation_error);
...
function wpcf7_add_tag_generator_quiz() {
    ...
    wpcf7_add_tag_generator('quiz', __('Quiz', 'contact-form-7'), 'wpcf7-tg-pane-quiz', 'wpcf7_tg_pane_quiz');
}
