...
function wpcf7_textarea_shortcode_handler($tag) {
    ...
    $html = sprintf(
            '<span class="wpcf7-form-control-wrap %1$s"><textarea %2$s>%3$s</textarea>%4$s</span>', sanitize_html_class($tag->name), $atts, esc_textarea($value), $validation_error);
...
function wpcf7_add_tag_generator_textarea() {
    ...
    wpcf7_add_tag_generator('textarea', __('Text area', 'contact-form-7'), 'wpcf7-tg-pane-textarea', 'wpcf7_tg_pane_textarea');
}
