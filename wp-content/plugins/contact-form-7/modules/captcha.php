...
function wpcf7_captcha_shortcode_handler($tag) {
    ...
    if ('captchac' == $tag->type) { // CAPTCHA-Challenge (image)
        ...
        $html = sprintf(
                '<input type="hidden" name="_wpcf7_captcha_challenge_%1$s" value="%2$s" /><img %3$s />', $tag->name, $prefix, $atts);
...
function wpcf7_add_tag_generator_captcha() {
    ...
    wpcf7_add_tag_generator('captcha', __('CAPTCHA', 'contact-form-7'), 'wpcf7-tg-pane-captcha', 'wpcf7_tg_pane_captcha');
}
