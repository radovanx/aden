...
function wpcf7_add_shortcode_checkbox() {
    wpcf7_add_shortcode(array('checkbox', 'checkbox*', 'radio'), 'wpcf7_checkbox_shortcode_handler', true);
}
...
function wpcf7_checkbox_shortcode_handler($tag) {
    ...
    foreach ($values as $key => $value) {
        ...
        if ($label_first) { // put label first, input last
            $item = sprintf(
                    '<span class="wpcf7-list-item-label">%1$s</span>&nbsp;<input %2$s />', esc_html($label), $item_atts);
        } else {
            $item = sprintf(
                    '<input %2$s />&nbsp;<span class="wpcf7-list-item-label">%1$s</span>', esc_html($label), $item_atts);
        }
...
function wpcf7_add_tag_generator_checkbox_and_radio() {
    ...
    wpcf7_add_tag_generator('checkbox', __('Checkboxes', 'contact-form-7'), 'wpcf7-tg-pane-checkbox', 'wpcf7_tg_pane_checkbox');

    wpcf7_add_tag_generator('radio', __('Radio buttons', 'contact-form-7'), 'wpcf7-tg-pane-radio', 'wpcf7_tg_pane_radio');
}
