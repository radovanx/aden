...
function wpcf7_add_shortcode_select() {
    wpcf7_add_shortcode(array('select', 'select*'), 'wpcf7_select_shortcode_handler', true);
}
...
function wpcf7_select_shortcode_handler($tag) {
    ...
    $html = sprintf(
            '<span class="wpcf7-form-control-wrap %1$s"><select %2$s>%3$s</select>%4$s</span>', sanitize_html_class($tag->name), $atts, $html, $validation_error);
...
function wpcf7_add_tag_generator_menu() {
    ...
    wpcf7_add_tag_generator('menu', __('Drop-down menu', 'contact-form-7'), 'wpcf7-tg-pane-menu', 'wpcf7_tg_pane_menu');
}
