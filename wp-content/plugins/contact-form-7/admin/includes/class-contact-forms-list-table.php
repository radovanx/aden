    ...
    function column_title($item) {
        ...
        if (current_user_can('wpcf7_edit_contact_form', $item->id)) {
            $copy_link = wp_nonce_url(
                    add_query_arg(array('action' => 'copy'), $url), 'wpcf7-copy-contact-form_' . absint($item->id));

            $actions = array_merge($actions, array(
                'copy' => '<a href="' . $copy_link . '">' . __('Copy', 'contact-form-7') . '</a>'));
        }

        $a = sprintf('<a class="row-title" href="%1$s" title="%2$s">%3$s</a>', $edit_link, esc_attr(sprintf(__('Edit &#8220;%s&#8221;', 'contact-form-7'), $item->title)), esc_html($item->title));
