var PourOver = (function() {
    ...
    PourOver = {
        ...
        build_permutation_array: function(set, sort) {
            ...
            _(sorted_set).each(function(m, i) {
                perm[m.cid] = i;
            });
            return perm;
        },
        ...
        cacheMethods: {
            ...
            defaultCache: function(items) {
                ...
                _(that.possibilities).each(function(p) {
                    var matching_items = _(items).filter(function(i) {
                        return that.fn(p, i);
                    }),
                            matching_cids = _(matching_items).map(function(i) {
                        return i.cid;
                    });
    ...
    _.extend(PourOver.Collection.prototype, PourOver.Events, {
        ...
        addFilters: function(f) {
            ...
            new_filters = _(f).reduce(function(m, i) {
                m[i.name] = _.clone(i);
        ...
        getAllItems: function() {
            var cids = _(this.items).map(function(i) {
                return i.cid;
            });
        ...
        getFilteredItems: function(filter_name, query) {
            ...
            if (_.isUndefined(filter))
                throw "The filter " + filter_name + " does not exist.";
        ...
        getItemValue: function(cid, attribute) {
            var item = _(this.items).find(function(i) {
    ...
    PourOver.Item = function(i) {
        return i;
            ...
            PourOver.Filter = function(name, values, opts) {
                ...
                this.values = _(values).map(function(v) {
                    return v.value;
                });
                _.extend(this, opts);
                this.initialize.apply(this, arguments);
    ...
    _.extend(PourOver.Filter.prototype, PourOver.Events, {
        ...
        removeFromCache: function(items) {
            var cids = _(items).map(function(i) {
                return i.cid;
            }).sort(function(a, b) {
                return a - b;
            });
    ...
    PourOver.View = function(name, collection, opts) {
        ...
        this.match_set = new PourOver.MatchSet(_(this.collection.items).map(function(i) {
            return i.cid;
        }), this.collection, ["all"]);
        if (opts.template) {
    ...
    _.extend(PourOver.View.prototype, PourOver.Events, {
        ...
        setSort: function(sort_name, view_sort) {
            ...
            if (sort_name && view_sort) {
                ...
                this.current_sort.on("resort", function() {
        ...
        getCurrentItems: function(page) {
            ...
            if (this.page_size == Infinity) {
                ...
                if (this.current_sort) {
                    ...
                    var ordered_cids = _(items).clone().sort(function(a, b) {
                        return a - b;
                    });
                    var unsorted_items = this.collection.get(ordered_cids);
                    items = _(items).map(function(i) {
                        return _(unsorted_items).find(function(o) {
                            return o.cid === i;
                        });
                    });
                } else {
        ...
        page: function(dir) {
            ...
            if (new_dir < 0)
                new_dir = 0;
            if (new_dir > Math.ceil(this.match_set.length() / this.page_size - 1))
                new_dir = Math.ceil(this.match_set.length() / this.page_size - 1);
        ...
        setPage: function(page) {
            if (page < 0)
                page = 0;
            if (page > Math.ceil(this.match_set.length() / this.page_size - 1))
                page = Math.ceil(this.match_set.length() / this.page_size - 1);
            this.current_page = page;
            this.trigger("pageChange");
        },
        // Set the page size.
    ...
    _.extend(PourOver.MatchSet.prototype, PourOver.Events, {
        ...
        refresh: function(s, match_set) {
            ...
            if (typeof (operation) === "object") {
                ...
                var cids = _(this.collection.items).map(function(i) {
                    return i.cid;
                });
    ...
    PourOver.BufferedCollection = PourOver.Collection.extend({
        ...
        stripFutures: function(item) {
            return _(item).reduce(function(m, v, k) {
        ...
        getFn: function(query) {
            ...
            if (_(query).isArray()) {
                ...
                if (_.isUndefined(possibility))
                    throw "The filter " + this.name + " does not have a match for the query '" + query + "'.";
    ...
    PourOver.makeRangeFilter = function(name, ranges, opts) {
        ...
        var values = _(ranges).map(function(r) {
            return {low: r[0], high: r[1], value: r.join("-")}
        }),
                attr = opts.attr || name,
    ...
    PourOver.dvrangeFilter = PourOver.Filter.extend({
        ...
        getFn: function(query) {
            ...
            that = this;
            possibilities = _(this.values.slice(li, hi + 1)).map(function(p) {
                return that.possibilities[p]
            });
            cids = _(possibilities).reduce(function(m, i) {
    ...
    PourOver.explicitSort = PourOver.Sort.extend({
        ...
        move: function(items, index) {
            ...
            var attrs = _(items).pluck(this.attr);
            this.order = _(this.order).map(function(o) {
                return _(attrs).include(o) ? null : o
            });
