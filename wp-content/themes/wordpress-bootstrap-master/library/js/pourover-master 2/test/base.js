...
test("permutation", function() {
    var permutation_array = PourOver.build_permutation_array([{cid: 1, name: "Erik"}, {cid: 2, name: "Brad"}, {cid: 3, name: "Chris"}], function(a, b) {
...
test("Sorts can be created", function() {
    var collection = new PourOver.Collection([{gender: "boy", name: "Erik", age: 26, color: "red"}, {gender: "boy", name: "Bart", age: 100, color: "dead"}, {name: "Cindy", age: 10, color: "blue", gender: "girl"}, {gender: "girl", name: "Sandra", age: 70, color: "purple"}, {gender: "boy", name: "Cargo", age: 10, color: "gold"}]),
            filter = PourOver.makeExactFilter("gender", ["boy", "girl"]),
            age_sort = PourOver.Sort.extend({fn: function(a, b) {
                    return a.age - b.age
                }}),
            sort = new age_sort("age");
    collection.addFilters([filter]);
    collection.addSort(sort);
    var items = _(collection.getSortedItems("age")).map(function(i) {
        return i.cid
    });
...
test("Views can be sorted", function() {
    var collection = new PourOver.Collection([{gender: "boy", name: "Erik", age: 26, color: "red"}, {gender: "boy", name: "Bart", age: 100, color: "dead"}, {name: "Cindy", age: 10, color: "blue", gender: "girl"}, {gender: "girl", name: "Sandra", age: 70, color: "purple"}, {gender: "boy", name: "Cargo", age: 10, color: "gold"}]),
            filter = PourOver.makeExactFilter("gender", ["boy", "girl"]),
            age_sort = new PourOver.Sort("age", {fn: function(a, b) {
                    return a.age - b.age
                }});
    collection.addFilters([filter]);
...
test("100000 items can be fast filtered", function() {
    fixture = [], i = 0;
    while (i < 100000) {
        fixture.push({quantity: Math.random() * 3 >>> 0, total: Math.random() * 300 >>> 0, tip: Math.random() * 200 >>> 0, type: ["tab", "visa", "cash"][Math.random() * 3 >>> 0], color: ["red", "orange", "yellow", "green", "blue", "indigo", "violet"][Math.random() * 7 >>> 0]});
        i++;
    }
...
test("Queries can be unioned", function() {
    ...
    ok(_.any(items, function(i) {
        return i.color == "blue"
    }), "Union adds the correct items to the query")
    var change_item = _(c.items).find(function(i) {
        return i.color == "indigo"
    })
...
test("Queries can be intersected", function() {
    ...
    var items = c.get(c.filters.color.current_query.cids);
    ok(_.all(items, function(i) {
        return i.color == "red"
    }), "Intersecting works properly")
    var change_item = _(c.items).find(function(i) {
        return i.color == "blue"
    })
...
test("Queries can be subtracted", function() {
    ...
    var items = c.get(c.filters.color.current_query.cids);
    ok(!_.any(items, function(i) {
        return i.color == "orange" || i.color == "blue" || i.color == "indigo" || i.color == "violet"
    }), "Subtracting corrent differences the query")
    var change_item = _(c.items).find(function(i) {
        return i.color == "red"
    })
